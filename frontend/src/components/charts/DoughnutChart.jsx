...
/** Allocation list sort — `null` keeps holdings order after name filter. */
const ALLOCATION_SORT = {
  NAME_ASC: 'name-asc',
  NAME_DESC: 'name-desc',
  PCT_DESC: 'pct-desc',
  PCT_ASC: 'pct-asc',
...
const ALLOCATION_SORT_OPTIONS = [
  { id: 'original', value: null, label: 'Original order' },
  { id: ALLOCATION_SORT.NAME_ASC, value: ALLOCATION_SORT.NAME_ASC, label: 'Name (A–Z)' },
  { id: ALLOCATION_SORT.NAME_DESC, value: ALLOCATION_SORT.NAME_DESC, label: 'Name (Z–A)' },
  {
    id: ALLOCATION_SORT.PCT_DESC,
    value: ALLOCATION_SORT.PCT_DESC,
    label: 'Percentage (high → low)',
  },
  { id: ALLOCATION_SORT.PCT_ASC, value: ALLOCATION_SORT.PCT_ASC, label: 'Percentage (low → high)' },
]
...
function FunnelIcon({ className }) {
  return (
    <svg
      ...
      aria-hidden
    >
      <path d="M4 5h16l-6 7v6l-2 1v-7L4 5z" />
    </svg>
  )
}
...
const DoughnutChart = ({ holdings, darkMode = false }) => {
  ...
  const [allocationSort, setAllocationSort] = useState(null)
  const [sortMenuOpen, setSortMenuOpen] = useState(false)

  const rootRef = useRef(null)
  const chartBlockRef = useRef(null)
  const allocationSortRef = useRef(null)
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e) => {
      if (allocationSortRef.current && !allocationSortRef.current.contains(e.target)) {
  ...
  useEffect(() => {
    const onWindowPointerDownCapture = (e) => {
      const t = e.target
      if (!(t instanceof Node) || !rootRef.current) {
        return
      }
      ...
      if (t instanceof Element && t.closest('ul[role="listbox"] button')) {
        return
      }
      if (chartBlockRef.current?.contains(t)) {
        return
      }
      setSelectedIndex(null)
    }
  ...
  const filteredAllocationRows = useMemo(() => {
    ...
    if (allocationSort === ALLOCATION_SORT.NAME_ASC) {
      rows = [...rows].sort((a, b) =>
        a.item.title.localeCompare(b.item.title, undefined, { sensitivity: 'base' })
      )
    } else if (allocationSort === ALLOCATION_SORT.NAME_DESC) {
      rows = [...rows].sort((a, b) =>
        b.item.title.localeCompare(a.item.title, undefined, { sensitivity: 'base' })
      )
    } else if (allocationSort === ALLOCATION_SORT.PCT_DESC) {
      rows = [...rows].sort((a, b) => pctAt(b.index) - pctAt(a.index))
    } else if (allocationSort === ALLOCATION_SORT.PCT_ASC) {
      rows = [...rows].sort((a, b) => pctAt(a.index) - pctAt(b.index))
    }
    ...
    >
      <div className="flex w-full flex-col gap-6 lg:flex-row lg:items-start lg:gap-6">
        ...
        <div className="order-2 flex w-full shrink-0 flex-col gap-2 lg:order-1 lg:w-52 lg:min-w-0 lg:max-w-full xl:w-56">
          ...
          {selectedIndex !== null &&
            holdings[selectedIndex] !== undefined &&
            holdings[selectedIndex] !== null && (
              <div
          ...
          <p className="ease-theme text-xs leading-snug text-stone-500 dark:text-gray-500">
            Click a slice or row to focus it; other positions merge into one neutral slice. Click
            the chart again to show the full breakdown.
          </p>
        </div>
        ...
        <div
          ...
          onClickCapture={(e) => {
            ...
            const t = e.target
            const fromListRow = t instanceof Element && !!t.closest('ul[role="listbox"] button')
            const fromSortUi = t instanceof Element && !!t.closest('[data-allocation-sort-anchor]')
            if (fromListRow || fromSortUi) {
              return
            }
        ...
        >
          ...
          <div className="relative shrink-0" ref={allocationSortRef} data-allocation-sort-anchor>
            <input
              type="text"
              placeholder="Filter holdings by name..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="ease-theme w-full rounded-lg border border-[color:var(--lm-border)] bg-[var(--lm-input)] py-2.5 pl-3 pr-11 text-sm text-stone-900 placeholder-stone-500 transition-shadow focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-950 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:ring-blue-400"
              aria-describedby="allocation-sort-hint"
            />
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-1.5">
              <button
                type="button"
                onClick={() => setSortMenuOpen((o) => !o)}
                className={`pointer-events-auto ease-theme flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-transparent text-stone-500 transition-colors duration-[var(--theme-transition-duration)] ease-[var(--theme-transition-ease)] hover:bg-black/[0.06] hover:text-stone-800 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700/80 dark:hover:text-gray-100 dark:focus:ring-blue-400 ${
                  allocationSort ? 'text-blue-600 dark:text-blue-400' : ''
                }`}
            ...
            {sortMenuOpen && (
              <div
                className="ease-theme absolute right-0 top-full z-20 mt-1.5 min-w-[13.5rem] overflow-hidden rounded-lg border border-[color:var(--lm-border)] bg-[var(--lm-card)] py-1 text-sm shadow-lg shadow-stone-900/15 dark:border-gray-600 dark:bg-gray-800 dark:shadow-black/40"
                role="menu"
                aria-label="Sort by"
              >
                {ALLOCATION_SORT_OPTIONS.map((opt) => {
                  const checked =
                    (opt.value === null && allocationSort === null) || opt.value === allocationSort
                  return (
                    <button
                      key={opt.id}
                      type="button"
                      ...
                      onClick={() => {
                        setAllocationSort(opt.value)
                        setSortMenuOpen(false)
                      }}
                      className={`ease-theme flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left ${
                        checked
                          ? 'bg-blue-50 font-medium text-blue-900 dark:bg-blue-950/50 dark:text-blue-100'
                          : 'text-stone-800 hover:bg-stone-100 dark:text-gray-200 dark:hover:bg-gray-700'
                      }`}
                    >
                      <span
            ...
            <span id="allocation-sort-hint" className="sr-only">
              Use the sort button to order by name or percentage.
            </span>
          </div>
          <ul
            ...
            role="listbox"
            aria-label={
              searchTerm.trim() ? 'Holdings matching your search' : 'Holdings allocation legend'
            }
          >
            {filteredAllocationRows.length === 0 ? (
              <li
                className="ease-theme px-3 py-4 text-center text-sm text-stone-500 dark:text-gray-500"
                role="status"
              >
